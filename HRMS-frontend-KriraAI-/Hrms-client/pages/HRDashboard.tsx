...
import { formatDate, formatDuration, getTodayStr, convertToDDMMYYYY, convertToYYYYMMDD, calculateBondRemaining, parseDDMMYYYY, calculateDailyTimeStats } from '../services/utils';
...
const MIN_NORMAL_SECONDS = (8 * 3600); // 8h = 28800 seconds
const MAX_NORMAL_SECONDS = (8 * 3600); // 8h = 28800 seconds
...
export const HRDashboard: React.FC = () => {
  ...
  // Build holiday Set for efficient lookup - memoized to avoid redundant rebuilds
  const holidayDateSet = useMemo(() => new Set(
    companyHolidays.map(h => typeof h.date === 'string' ? h.date.split('T')[0] : new Date(h.date).toISOString().split('T')[0])
  ), [companyHolidays]);

  const employeeStats = useMemo(() => users.filter(u => u.role === Role.EMPLOYEE).map(user => {
    ...
    records.forEach(r => {
      if (r.totalWorkedSeconds > 0 || (r.checkIn && !r.checkOut)) {
        ...
        if (extraTimeLeave && extraTimeLeave.startTime && extraTimeLeave.endTime) {
          ...
          netWorkedSeconds += leaveDays * 8 * 3600;
        }
    ...
    return {
      ...
      allLeaves,
      balance,
      halfDayLeaves: leaves.filter(l => l.category === LeaveCategory.HALF_DAY),
      extraTimeLeaves: leaves.filter(l => l.category === LeaveCategory.EXTRA_TIME)
    };
  }), [users, attendanceRecords, leaveRequests, holidayDateSet]);
  ...
  const calculateMonthlyStats = () => {
    ...
    // Normal time: 8:15 to 8:22, Low < 8:15, Extra > 8:22
    const MIN_NORMAL_SECONDS_LOCAL = 8 * 3600; // 8h = 28800 seconds
    const MAX_NORMAL_SECONDS_LOCAL = 8 * 3600; // 8h = 28800 seconds
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                          ...
                                          <tr key={l.id} className="border-t hover:bg-gray-50">
                                            ...
                                            <td className="px-3 py-2 text-center">
                                              {(l.status === LeaveStatus.APPROVED || l.status === LeaveStatus.REJECTED) && (
                                                <button
                                                  onClick={async () => {
                                                    if (!confirm(`Are you sure you want to revert this ${l.status.toLowerCase()} leave?`)) return;
                                                    try {
                                                      await updateLeaveStatus(l.id, LeaveStatus.PENDING, `Reverted from ${l.status} by HR`);
                            ...
                            <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                              <div className="px-4 py-2 bg-red-50 border-b border-red-100 font-semibold text-xs text-red-800 uppercase flex items-center gap-2">
                                <AlertCircle size={14} /> Low Time Logs (&lt; 8h)
                              </div>
                              {(() => {
                                const lowTimeRecords = stat.records.filter(r => {
                                  ...
                                  const netWorkedRaw = Math.floor((checkOut - checkInDate.getTime()) / 1000) - breakSec;
                            ...
                            <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                              <div className="px-4 py-2 bg-green-50 border-b border-green-100 font-semibold text-xs text-green-800 uppercase flex items-center gap-2">
                                <Clock size={14} /> Extra Time Logs (&gt; 8h)
                              </div>
                              {(() => {
                                const extraTimeRecords = stat.records.filter(r => {
                                  if (!r.checkIn || !r.checkOut) return false;
                                  const checkInDate = new Date(r.checkIn).getTime();
                                  const checkInDateObj = new Date(r.checkIn);
                                  const checkOut = new Date(r.checkOut).getTime();
                                  const breakSec = getBreakSeconds(r.breaks) || 0;
                                  const netWorkedRaw = Math.floor((checkOut - checkInDate) / 1000) - breakSec;
                    ...
                    {monthlyAttendance.length === 0 ? (
                      <tr>
                        ...
                        monthlyAttendance.map((record, idx) => {
                          ...
                          const MIN_NORMAL_SECONDS_LOCAL = 8 * 3600 - adjustmentSeconds;
                          const MAX_NORMAL_SECONDS_LOCAL = 8 * 3600 - adjustmentSeconds;
          ...
          <div className="space-y-8">
            ...
            <section className="border-l-4 border-green-500 pl-6">
              ...
              <div className="space-y-3 text-gray-700">
                ...
                <p className="flex items-start gap-2">
                  <ArrowRight className="h-4 w-4 text-green-500 mt-1 flex-shrink-0" />
                  <span><strong>Performance Flags:</strong> Low time flag (less than 8h) and extra time flag (more than 8h) help track employee performance.</span>
                </p>
              </div>
            </section>
