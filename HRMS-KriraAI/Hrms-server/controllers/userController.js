...
export const getAllUsers = async (req, res) => {
  try {
    ...
    // Tenant isolation: Admin and HR only see users within their CareerLens tenant
    if (req.user.careerLensUserId) {
...
export const createUser = async (req, res) => {
  try {
    ...
    if (bonds && Array.isArray(bonds) && bonds.length > 0) {
      formattedBonds = bonds.map((bond, index) => {
        ...
        return {
          ...
          endDate: bondEndDate,
          order: index + 1,
          salary: parseFloat(bond.salary) || 0
        };
    ...
    const user = new User({
      ...
      // Inherit tenant ID from the creating admin so this user stays in the same tenant
      careerLensUserId: currentUser.careerLensUserId || null,
    });
...
export const getUsersByRole = async (req, res) => {
  try {
    ...
    // Tenant isolation: scope by careerLensUserId if available
...
export const deleteUser = async (req, res) => {
  try {
    ...
    if (currentUser.careerLensUserId && user.careerLensUserId !== currentUser.careerLensUserId) {
      return res.status(403).json({ message: 'Access denied. Cannot delete users from another tenant.' });
    }
...
export const resetAllPaidLeaveAllocation = async (req, res) => {
  try {
    ...
    // Reset all employees' and HR's paidLeaveAllocation to 0 (tenant-scoped)
    const tenantFilter = { role: { $in: ['Employee', 'HR'] }, isActive: true };
    if (currentUser.careerLensUserId) {
      tenantFilter.careerLensUserId = currentUser.careerLensUserId;
    }
...
export const getEmployeeStats = async (req, res) => {
  try {
    ...
    // Tenant isolation: scope stats to caller's tenant
    if (req.user && req.user.careerLensUserId) {
      employeeFilter.careerLensUserId = req.user.careerLensUserId;
    }
