...
// Helper: check if a date string (YYYY-MM-DD) is a company holiday (scoped to tenant)
const checkIsHoliday = async (dateStr, tenantId = null) => {
  const filter = { date: dateStr };
  if (tenantId) filter.careerLensUserId = tenantId;
  const holiday = await CompanyHoliday.findOne(filter);
  return !!holiday;
};

// Helper: get all user _ids belonging to a specific tenant
...
export const clockOut = async (req, res) => {
  try {
    ...
    // Check if today is a company holiday (scoped to tenant)
    const isHolidayWork = await checkIsHoliday(today, req.user.careerLensUserId || null);
    ...
    const { lowTime, extraTime } = getFlags(worked, !!hasHalfDay, extraTimeLeaveMinutes, isHolidayWork);
  ...
  try {
    ...
    for (const record of attendance) {
      if (record.checkIn && record.checkOut && (record.lowTimeFlag === undefined || record.extraTimeFlag === undefined || record.lowTimeFlag === null || record.extraTimeFlag === null)) {
        ...
        const flags = getFlags(worked, !!hasHalfDay, 0, false);
...
export const adminCreateAttendance = async (req, res) => {
  try {
    ...
    if (attendance) {
      ...
      const isHolidayWork = await checkIsHoliday(attendance.date, req.user.careerLensUserId || null);
        ...
        flags = getFlags(worked, !!hasHalfDay, extraTimeLeaveMinutes, isHolidayWork);
        attendance.totalWorkedSeconds = worked;
      ...
      const logMessage = `Modified attendance record for ${date}`;
    ...
    if (checkInDate && checkOutDate) {
      ...
      const isHolidayWork = await checkIsHoliday(date, req.user.careerLensUserId || null);
    ...
    const logMessage = `Created attendance record for ${date}`;
...
export const adminUpdateAttendance = async (req, res) => {
  try {
    ...
    const { checkIn, checkOut, breakDurationMinutes, notes, isManualFlag, lowTimeFlag, extraTimeFlag, totalWorkedSeconds: bodyTotalWorkedSeconds, workedHours: bodyWorkedHours } = req.body;
    ...
    if (!attendance.isManualFlag && attendance.checkIn && attendance.checkOut) {
      ...
      const flags = getFlags(worked, !!hasHalfDay, extraTimeLeaveMinutes, isHolidayWork);
      attendance.totalWorkedSeconds = worked;
      attendance.lowTimeFlag = flags.lowTime;
      attendance.extraTimeFlag = flags.extraTime;
...
export const getAllAttendance = async (req, res) => {
  try {
    ...
    // Tenant isolation: restrict attendance records to users in the same tenant
    const tenantId = req.user ? req.user.careerLensUserId || null : null;
    ...
    // Build a Set of all holiday date strings scoped to this tenant
    const allHolidays = await CompanyHoliday.find(tenantId ? { careerLensUserId: tenantId } : {}, 'date').lean();
    ...
    // Now process records in-memory â€” zero additional DB calls
    const recordsToSave = [];
...
export const getTodayAllAttendance = async (req, res) => {
  try {
    const today = getTodayStr();
    // Tenant isolation: only fetch today's attendance for users in the same tenant
    const tenantId = req.user ? req.user.careerLensUserId || null : null;
    const tenantUserIds = await getTenantUserIds(tenantId);
    const attendance = await Attendance.find({ date: today, userId: { $in: tenantUserIds } })
      .populate('userId', 'name username email department role')
      .sort({ checkIn: 1 });

    // --- OPTIMIZED: Bulk pre-fetch holidays and leaves ONCE before the loop ---
    const isHolidayWork = !!(await CompanyHoliday.findOne({ date: today, ...(tenantId ? { careerLensUserId: tenantId } : {}) }).lean());
