...
// POST /api/auth/register-admin  (called internally from CareerLens server)
export const registerAdmin = async (req, res) => {
  try {
    const HRMS_INTERNAL_SECRET = process.env.HRMS_INTERNAL_SECRET || 'careerlens-hrms-secret-2024';
    const providedSecret = req.headers['x-internal-secret'];

    if (!providedSecret || providedSecret !== HRMS_INTERNAL_SECRET) {
      return res.status(403).json({ message: 'Unauthorized: invalid internal secret' });
    }

    const { name, username, email, password, careerLensUserId, department } = req.body;

    if (!name || !username || !email || !password || !careerLensUserId) {
      return res.status(400).json({ message: 'Missing required fields: name, username, email, password, careerLensUserId' });
    }

    // Check if username already taken by this tenant
    const existing = await User.findOne({ username: username.toLowerCase(), careerLensUserId });
    if (existing) {
      return res.status(409).json({ message: 'Username already exists for this account. Please choose a different one.' });
    }
    ...
    res.status(201).json({
      message: 'HRMS Admin account created successfully',
      user: userObj
    });
  } catch (error) {
